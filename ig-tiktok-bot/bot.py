...
# Webhook mode (for serverless/scale-to-zero hosts like Cloud Run). If unset,
# the bot falls back to long polling, which is the right choice for running
...
def main() -> None:
    ...
    if WEBHOOK_URL:
        # Serverless / scale-to-zero mode: Telegram pushes updates to us via
        # HTTP instead of us constantly asking for them. The token in the
        # URL path keeps randos from POSTing fake updates to the endpoint;
        # WEBHOOK_SECRET adds a second check via Telegram's secret-token
        # header if set.
        log.info("Starting webhook on port %s -> %s/<token>", PORT, WEBHOOK_URL)
