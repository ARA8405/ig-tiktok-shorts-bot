...
there." If you want to stop needing your own machine on at all, skip to
"Deploy to Google Cloud Run" below — it's free for this bot's traffic and
needs no computer of yours running.
...
## 7. Deploy to Google Cloud Run (free, no computer of yours required)

Cloud Run runs your same Docker container in the cloud, but **scales to
zero** — nothing runs (and nothing is charged) when no one's sending
links, and it wakes up in a couple of seconds when a message comes in.
Cloud Run's free tier comfortably covers a personal bot's traffic. This
mode uses **webhooks** instead of polling (Telegram pushes updates to your
bot instead of your bot constantly asking for them), which the bot
already supports via the `WEBHOOK_URL` setting in `.env`.

You'll need a Google account and the [`gcloud` CLI](https://cloud.google.com/sdk/docs/install) installed.

1. **Create a Google Cloud project** (skip if you have one already):
   ```bash
   gcloud projects create ig-tiktok-bot-YOURNAME
   gcloud config set project ig-tiktok-bot-YOURNAME
   gcloud services enable run.googleapis.com
   ```

2. **First deploy** (from this project folder). Env vars are passed with
   `--set-env-vars` instead of `.env` (Cloud Run doesn't read `.env`
   files) — use your real token here:
   ```bash
   gcloud run deploy ig-tiktok-bot \
     --source . \
     --region us-central1 \
     --allow-unauthenticated \
     --set-env-vars TELEGRAM_BOT_TOKEN=your_real_token_here
   ```
   This builds the container from your `Dockerfile` and deploys it. At
   the end it prints a **Service URL** like
   `https://ig-tiktok-bot-xxxxx-uc.a.run.app` — copy it.

3. **Point the bot at its own URL** (needed so it knows what URL to
   register as its webhook with Telegram):
   ```bash
   gcloud run services update ig-tiktok-bot \
     --region us-central1 \
     --set-env-vars WEBHOOK_URL=https://ig-tiktok-bot-xxxxx-uc.a.run.app
   ```
   This restarts the service, and the bot registers its webhook
   automatically on startup — no manual Telegram API call needed.

4. **Test it** — post a link in your group. The first message after a
   period of inactivity may take a couple of seconds longer than usual
   (Cloud Run "cold starting" the container) — that's expected and normal.

**Updating the bot later:** re-run the same `gcloud run deploy` command
from step 2 (without `--set-env-vars`, so it keeps your existing settings)
whenever you change `bot.py`.

**Cost:** Cloud Run's always-free tier includes far more requests and
compute time per month than a personal group chat bot will use. Google
does require a billing account on file (for identity verification), but
you won't be charged unless you drastically exceed the free tier.
